...
#
# Runs the grep-based subset of the pipeline: every rule whose detection is
# pure regex / string match. Subjective phases (security judgment, design
# critique, a11y heuristics that need rendering) run through the slash
# command in Claude Code.
#
# Exit codes:
#   0   all deterministic checks passed
#   1   at least one P0 or P1 finding
#   2   usage error, missing dependency, or bad config
#
# Env vars:
#   QC_ROOT     repo root to scan (default: pwd)
#   QC_TARGETS  space-separated paths to scan (default: auto-detected)
#   QC_JSON     path to write JSON findings to (default: stdout summary only)
#   QC_STRICT   "1" to fail on P2 as well as P0/P1 (default: 0)
#   QC_QUIET    "1" to suppress per-finding output (default: 0)
#
# Designed to be sourced from CI (.github/workflows/quality-checks.yml) and
# from the slash command's pre-flight step. Idempotent. No writes outside
# QC_JSON.
...
# Auto-detect scan targets if not provided.
...
emit_finding() {
  ...
  if [ -n "$JSON" ]; then
    ...
    local obj
    obj=$(printf '{"id":"%s","severity":"%s","label":"%s","file":"%s","line":%s,"snippet":"%s"}' \
      "$id" "$sev" "$label" "$file" "$line" "$esc_snip")
    if [ -z "$FINDINGS_JSON" ]; then
      FINDINGS_JSON="$obj"
    else
      FINDINGS_JSON="$FINDINGS_JSON,$obj"
    fi
  fi
}
...
# Phase 2 — Anti-pattern absolutes (regex-detectable subset)
...
# Phase 4 — A11y (regex-detectable subset)
...
# Phase 8 — Stack hazards (regex-detectable subset)
...
if [ -n "$JSON" ]; then
  printf '{"version":"1.1.0","p0":%d,"p1":%d,"p2":%d,"findings":[%s]}' \
    "$TOTAL_P0" "$TOTAL_P1" "$TOTAL_P2" "$FINDINGS_JSON" > "$JSON"
fi
...
if [ "$FAIL" -eq 0 ]; then
  [ "$QUIET" != "1" ] && echo "Status: PASS"
  exit 0
else
  [ "$QUIET" != "1" ] && echo "Status: FAIL — run /quality-checks for the full report"
  exit 1
fi
