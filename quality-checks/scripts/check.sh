...
# Honors .qcignore at QC_ROOT (one glob pattern per line, # comments).
...
if [ -z "${QC_TARGETS:-}" ]; then
  CANDIDATES=()
  for d in src site app pages content components layouts public; do
    [ -e "$d" ] && CANDIDATES+=("$d")
  done
...
if [ -f ".qcignore" ]; then
  while IFS= read -r line; do
    ...
    if command -v rg >/dev/null 2>&1; then
      QC_IGNORE_GLOBS="$QC_IGNORE_GLOBS --glob=!$line"
    else
      QC_IGNORE_GLOBS="$QC_IGNORE_GLOBS --exclude=$line"
    fi
  done < ".qcignore"
fi
...
if command -v rg >/dev/null 2>&1; then
  ...
  GREP_I="-i"
  GREP_EXCLUDE="--glob=!node_modules --glob=!.git --glob=!dist --glob=!build --glob=!.next --glob=!.astro --glob=!coverage --glob=!*.lock --glob=!*.lockb --glob=!quality-checks/** $QC_IGNORE_GLOBS"
else
  GREP_CMD="grep -E -r --line-number --color=never"
  GREP_I="-i"
  GREP_EXCLUDE="--exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist --exclude-dir=build --exclude-dir=.next --exclude-dir=.astro --exclude-dir=coverage --exclude-dir=quality-checks $QC_IGNORE_GLOBS"
fi
...
if [ "$QUIET" != "1" ]; then
  printf "\nQuality Checks (deterministic): P0=%d P1=%d P2=%d\n" "$TOTAL_P0" "$TOTAL_P1" "$TOTAL_P2"
fi

if [ -n "$JSON" ]; then
  printf '{"version":"1.2.0","p0":%d,"p1":%d,"p2":%d,"findings":[%s]}' "$TOTAL_P0" "$TOTAL_P1" "$TOTAL_P2" "$FINDINGS_JSON" > "$JSON"
fi
